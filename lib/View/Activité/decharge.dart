...
class DechargeCartePage extends StatefulWidget {
  const DechargeCartePage({super.key});

  @override
  _DechargeCartePageState createState() => _DechargeCartePageState();
}

class _DechargeCartePageState extends State<DechargeCartePage> {
  ...
  // Variables sécurisées pour les arguments
  String cardID = '';
  String cardType = '';
  String cardNumber = '';
  String holderName = '';
  ...
  void _initializeArguments() {
    try {
      ...
      if (arguments != null && arguments is Map<String, dynamic>) {
        ...
        cardType = arguments['cardType']?.toString() ?? '';
        cardNumber = arguments['cardNumber']?.toString() ?? '';
        holderName = arguments['holderName']?.toString() ?? '';
        isActive = arguments['isActive'] ?? false;
      } else {
        // Gérer le cas où aucun argument n'est passé
        WidgetsBinding.instance.addPostFrameCallback((_) {
          _showErrorDialog('Erreur: Informations de carte manquantes');
        });
      }
    } catch (e) {
      debugPrint('Erreur initialisation arguments: $e');
      WidgetsBinding.instance.addPostFrameCallback((_) {
        _showErrorDialog('Erreur lors du chargement des données de carte');
      });
    }
  }
  ...
  String? _validateAmount(String? value) {
    ...
    if (amount < 100) {
      // Corrigé: minimum 1000 FCFA
      return 'Le montant minimum est de 100 FCFA';
    }
    if (amount > 2000000) {
      // Corrigé: cohérence avec le message
      return 'Le montant maximum est de 2,000,000 FCFA';
    }
    return null;
  }

  Future<void> _processDecharge() async {
    ...
    // Vérification que la carte est active
    if (!isActive) {
      _showErrorDialog(
          'Cette carte n\'est pas active. Impossible d\'effectuer un retrait.');
      return;
    }
    ...
    try {
      ...
      debugPrint('Décharge - Numéro: $numero, Montant: $montant');

      final result = await RechargeDechargeService.dechargeCarte(
        cardNumber: cardID,
      ...
      debugPrint('Erreur pendant le retrait: $e');
      _showErrorDialog('Erreur lors du retrait. Veuillez réessayer.');
    } finally {
      if (mounted) {
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              Text(
                'Votre retrait de ${montantController.text} FCFA a été effectué avec succès depuis la carte ID: $cardID vers votre wallet',
                ...
                child: Row(
                  children: [
                    Icon(Icons.account_balance_wallet,
                        color: Colors.green[700], size: 4.w),
                    SizedBox(width: 2.w),
                    Expanded(
                      child: Text(
                        'Fonds ajoutés à votre wallet',
                        style: TextStyle(
  ...
  Widget _buildCardInfoSection(bool isSmallScreen) {
    return Container(
      padding: EdgeInsets.all(4.w),
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: [
            const Color(0xFF17338D).withOpacity(0.1),
        ...
        children: [
            ...
            children: [
              ...
              Container(
                ...
                child: Text(
                  ...
                  style: TextStyle(
                    color: Colors.white,
                    fontSize: isSmallScreen ? 10.sp : 8.sp,
                    fontWeight: FontWeight.bold,
                  ),
                ),
              ),
          ...
          Row(
            children: [
              ...
              Expanded(
                child: Text(
                  'Retrait de carte vers le wallet en toute sécurité',
                  style: TextStyle(
                    color: Colors.grey[600],
  ...
  Widget _buildQuickAmountGrid(bool isSmallScreen, bool isTablet) {
    return GridView.builder(
      ...
      gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
        ...
        mainAxisSpacing: 1.h,
        childAspectRatio: isTablet ? 3 : (isSmallScreen ? 2.5 : 3),
      ),
      itemCount: _quickAmounts.length,
      itemBuilder: (context, index) {
        final amount = _quickAmounts[index];
        final isSelected = montantController.text == amount.toString();

        return GestureDetector(
          onTap: (_isLoading || _isVerifying)
              ? null
          ...
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(
                color: isSelected ? globalColor : Colors.grey[300]!,
            ...
            child: Center(
              child: Text(
                ...
                style: TextStyle(
                  color: isSelected ? Colors.white : globalColor,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
              ...
              child: ConstrainedBox(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildCardInfoSection(isSmallScreen),
                    ...
                    Container(
                      ...
                      child: TextFormField(
                        ...
                        decoration: InputDecoration(
                          enabled: !(_isLoading || _isVerifying) ? true : false,
      ...
      //   print('📊📊 $numero');
      final isActive = await service.isFeatureActive(AppFeature.depotCarteWallet);

        if (isActive) {
          print('✅ La recharge Carte  wallet est disponible');
        } else {
          Get.back();
                   setState(() => _isVerifying = false);
